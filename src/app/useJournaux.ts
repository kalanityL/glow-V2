...
export function useJournaux() {
  const [tables, setTables] = useState(() => {
    ...
    const base = lireBase();
    return { prises: base.injectionHistory, pesees: base.weightHistory, sommeils: base.sleepLogs ?? [], activites: base.sportLogs ?? [] };
  });
  return {
    ...
    consignerActivite: (activite: SportLog): readonly SportLog[] => {
      ...
      setTables({ prises: base.injectionHistory, pesees: base.weightHistory, sommeils: base.sleepLogs ?? [], activites: base.sportLogs ?? [] });
